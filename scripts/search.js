...
function isLocationOk(job) {
  ...
  if ((job.source === 'Greenhouse' || job.source === 'Lever') &&
      (loc === 'united states' || loc === 'usa' || loc === '' || loc === 'north america')) return true;
  return false;
}
...
main().catch(err => {
  ...
  process.exit(1);
    ...
    'rippling','gusto','justworks','bamboohr','namely','paycor','paylocity','isolved',
    'leapsome','lattice','hibob','cultureamp','15five','personio','factorial','betterworks',
    'remote','oysterhr','deel','papayaglobal','velocityglobal','multiplier','remofirst','omnipresent','insperity',
    'checkr','firstadvantage','sterlingcheck','accurate','veriff','truework','hireright',
    'partnerstack','crossbeam','impartner','tackle','impact','partnerize','everflow','workato','zapier','celigo',
    'salesloft','outreach','gong','clari','mindtickle','highspot','seismic',
    'gainsight','totango','churnzero','planhat',
    'hubspot','zendesk','freshworks','intercom','drift','qualified',
    'navan','expensify','brex','ramp','airbase',
    'fivetran','hightouch','vanta','drata','secureframe',
    'servicenow','salesforce','experian'
  ];ick Stephen Job Search Agent - v13 - Intelligent Scoring
const fs = require('fs');
const path = require('path');
